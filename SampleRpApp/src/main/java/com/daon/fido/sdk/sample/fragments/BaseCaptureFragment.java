...
public abstract class BaseCaptureFragment extends Fragment implements VerificationAttemptParameters, EmbeddedView, CaptureArguments, ControllerAware, AuthenticationInstance {
    ...
    private final Handler parentActivityHandler = new Handler();
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        Log.d(TAG, "onCreate: Capture fragment created: " + this.getClass().getSimpleName());
        if (savedInstanceState != null) {
            Log.d(TAG, "ctrlr fragment onCreate: saved Instance state != null");
    ...
    public void onStart() {
        ...
        start();
        boolean overlayDetectionEnabled = GlobalSettings.getInstance().isOverlayDetectionEnabled();
    ...
    @Override
    public void onPause() {
        Log.d(TAG, "oPause: " + this.getClass().getSimpleName());
        super.onPause();
        stop();
    }
    ...
    public class DefaultCaptureCompleteListener implements CaptureCompleteListener {
        ...
        public void onCaptureComplete(CaptureCompleteResult result) {
            ...
            if (result.getError() != null) {
                Log.e(TAG, "onCaptureComplete: Error: " + result.getError());
            }
            if (getController() != null) {
                disableWaitUI();
                switch (result.getType()) {
                    case CaptureCompleteResult.Type.TERMINATE_SUCCESS -> onTerminateSuccess(result);
                    case CaptureCompleteResult.Type.TERMINATE_FAILURE -> onTerminateFailure(result);
                    ...
                    case CaptureCompleteResult.Type.CLIENT_ERROR -> onClientError(result);
                }
            }
        }
        ...
        protected void onClientAuthenticationFailed(CaptureCompleteResult result) {
            Log.e(TAG, "onClientAuthenticationFailed: " + result);
    ...
    protected void onAuthenticatorLocked(LockResult result) {
        Log.d(TAG, "onAuthenticatorLocked: ");
    ...
    protected void reportAuthenticatorLocked(LockResult result) {
        Log.d(TAG, "reportAuthenticatorLocked: ");
        if (showMessageOnTerminate()) {
            switch (result.getLockInfo().getState()) {
                case PERMANENT -> showMessage(R.string.authenticator_locked, false);
                case TEMPORARY ->
    ...
    protected void completeCaptureWithError(AuthenticatorError error) {
        Log.e(TAG, "completeCaptureWithError: " + error);
    ...
    protected void terminateParentActivity() {
        Log.d(TAG, "terminateParentActivity: ");
    ...
    protected void terminateParentActivityWithError(final int code, final String message) {
        Log.e(TAG, "terminateParentActivityWithError: " + message);
        parentActivityHandler.postDelayed(() -> {
            if (callback != null && getController() != null) {
    ...
    protected void initListeners() {
        CaptureActivity.MyOnTouchListener myOnTouchListener = event -> {
    ...
    protected void showOverlayAlert() {
        ...
        builder.setPositiveButton(R.string.obscure_window_positive_button, (dialogInterface, i) -> dialogInterface.dismiss());
    ...
    public void onDestroy() {
        Log.d(TAG, "onDestroy: Destroyed fragment:" + this.getClass().getSimpleName());
        super.onDestroy();
