...
/**
 * This is a view used to capture a pattern from the user.
 * The view is divided in 81 rectangles (9x9) and there
 * are 9 defined touch points (1-9).
 *
 * The rules are pretty simple
 *   1. A touch point can only be used once
 *   2. A single continuous pattern is required
 *   3. If a straight line between two points intersects a mid point, then the
 *      mid point is included
 *   4. The order, the touch points are recorded is important
 *
 *
 ...
 *
 *   There are two Modes and four States supported by the system.
 *
 *   The two Modes are ENROLL and AUTHENTICATE
 *
 *   ENROLL
 *   The
 */
public class PatternCollect extends View {
    ...
    public enum Status {FIRST_ENROLLMENT_COMPLETE, INVALID_CONFIRMATION_ENROLLMENT, VERIFIED, NOT_VERIFIED, INVALID_ENROLMENT_MIN_SIZE, INVALID_ENROLMENT_MAX_SIZE, INVALID_ENROLMENT_WEAK_PATTERN};
    public enum State {CAPTURE, FEEDBACK_POSITIVE, FEEDBACK_INVALID_INPUT};
    ...
    // Touched points
    private List<Integer> touchedPoints = new ArrayList<>();
    // Current start point
    ...
    // Touch boxes
    private RectF[] rectangles = new RectF[NUM_COLS * NUM_ROWS];
...
//    }
     ...
     *
     * If the user has not touched any touch points, then ignore the touch.
     *
     */
    ...
    protected void completeAuthentication(int[] pattern) {
        ...
        passcodeManager.validate(pattern, new IPatternManager.PatternValidListener() {
            @Override
            public void onPatternValid(boolean valid) {
                if (valid) {
    ...
    protected void processFirstEnrolment(int[] pattern) {
        ...
        if(controller != null) {
            ...
            if(error==null) {
                ...
                result.setAdditionalData(String.valueOf(pattern.length));
                sendDelayedResponse(result);
            } else {
                switch (error.getCode()) {
                    case ErrorCodes.ERROR_PATTERN_TOO_SHORT:
                        processEnrolmentError(Status.INVALID_ENROLMENT_MIN_SIZE, pattern);
                        return;
                    case ErrorCodes.ERROR_PATTERN_TOO_LONG:
                        processEnrolmentError(Status.INVALID_ENROLMENT_MAX_SIZE, pattern);
                        return;
                    case ErrorCodes.ERROR_PATTERN_WEAK:
                        processEnrolmentError(Status.INVALID_ENROLMENT_WEAK_PATTERN, pattern);
                        return;
                }
            }
        }
    ...
    protected void processEnrolmentError(Status failedStatus, int[] pattern) {
        PatternCollectResult result = new PatternCollectResult();
        result.setMode(Mode.ENROLL);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (parameters.showGridlines()) {
            for (RectF rect : rectangles)
                canvas.drawRect(rect, parameters.getGridLinePaint());
        }
        ...
        if (message != null && message.length() > 0) {
            ...
            int yPos = (int) ((canvas.getHeight() / 2) - (Math.abs(r.height()))/2 );    // or maybe -= instead of +=, depends on your coordinates
    ...
    protected void addTouchedPoints(int fromPoint, int toPoint) {
        ...
        if (rowDistance == 2 || columnDistance == 2) {
            if (rowDistance == 2 && columnDistance == 2) {
                ...
                if (rowDistance == 0 && columnDistance == 2) {
    ...
    protected int getRow(int aRectIndex) {
        ...
        switch (aRectIndex) {
            ...
            case 70:
                return 3;
            default:
                throw new RuntimeException("Invalid touchable rectangle at: " + aRectIndex);
        }
